...
//tomorrow chnage that logged in user is receiver as he has option to accept or reject
  ...
  try {
    ...
    if (!isUserSendedMessage) {
      return res.status(400).json({
        isSuccess: false,
        message:
          "user have not initiated any message , so you can sen your first message to user : " +
          isAlreadyFriend?.status,
      });
    }

    //i am logged in and getting optio to accpet or reject means
    // i am reciever and another person has send message to logged in user means me
